...
from cascadai.schema.token_piece import Token, Token_Type
...
def detect_tokens(model, image_path: str, conf: float = 0.6, imgsz: int = 1024) -> list[Token]:
    ...
    for r in results:
        for box in r.boxes:
            ...
            y_center = y1 + (y2 - y1) / 2
            width = ((x2 - x1) + (y2 - y1)) / 2
            token_type = Token_Type(cls)
            t = Token(token_type, x_center, y_center, width, conf_v)
            tokens.append(t)
    return tokens


def draw_detections(image_path, tokens: list[Token]):
    ...
    for token in tokens:
        ...
        x1, x2 = token.x - token.width // 2, token.x + token.width // 2
        y1, y2 = token.y - token.width // 2, token.y + token.width // 2
